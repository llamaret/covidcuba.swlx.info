...
$.getJSON("assets/data/paises-info-dias.json", function(countriesdays) {
  $.getJSON("assets/data/covid19-cuba.json", function(data) {
    $.getJSON("assets/data/provincias.geojson", function(provincias) {
      $.getJSON("assets/data/municipios.geojson", function(municipios) {
        ...
        function getAllCasesAndSimpleGraphics() {
          ...
          c3.generate({
            ...
            data: {
              columns: [
                ...
                ["Tests Negativos", total_tests - (total_cu + total_no_cu + total_unk)]
              ],
              type: "donut",
              colors: {
          ...
          tests = c3.generate({
            ...
            data: {
              x: ntest_days[0],
              columns: [ntest_days, ntest_negative, ntest_positive, ntest_cases],
              type: "bar",
          ...
          $("#countrycurve-select").on("change", function() {
            ...
            comparison = c3.generate({
              ...
              data: {
                x: dias[0],
                columns: [dias, cuba, curves[countryselected]["data"]],
                type: "line",
                colors: {
                  Cuba: "#520924"
                }
            ...
            curve = c3.generate({
              ...
              data: {
                x: "Días",
                columns: [curves[countryselected]["dias"], curves[countryselected]["data"], cuba],
                type: "line",
                colors: {
                  Cuba: "#520924"
                }
          ...
          for (var c in countriesdays.paises_info) {
            if (countriesdays.paises_info[c] && countriesdays.paises_info[c].confirmed.length > countriesdays.paises_info[c].deaths.length) {
              var fill_count = countriesdays.paises_info[c].confirmed.length - countriesdays.paises_info[c].deaths.length;
              countriesdays.paises_info[c].deaths = Array(fill_count)
                .fill(0)
                .concat(countriesdays.paises_info[c].deaths);
            }
            //  if (countriesdays.paises_info[c].length >= cuba.length) {
            var c_temp = [c];
            var d_temp = ["Días"];
            for (var i = 1; i < countriesdays.paises_info[c].deaths.length; i++) {
              c_temp.push(countriesdays.paises_info[c].deaths[i]);
              d_temp.push("Día " + i);
            }
          ...
          var generate_death_comparison = function(country) {
            return c3.generate({
              ...
              data: {
                x: dias[0],
                columns: [dias, deaths_cuba, death_curve[death_countryselected]["data"].slice(0, cuba.length)],
                type: "line",
                colors: {
                  Cuba: "#520924"
                }
          ...
          var generate_death_graph = function() {
            return c3.generate({
              ...
              data: {
                x: "Días",
                columns: [death_curve[death_countryselected]["dias"], deaths_cuba, death_curve[death_countryselected]["data"]],
                type: "line",
                colors: {
                  Cuba: "#520924"
                }
          ...
          for (var c in countriesdays.paises_info) {
            if (countriesdays.paises_info[c] && countriesdays.paises_info[c].confirmed.length > countriesdays.paises_info[c].recovered.length) {
              var fill_count = countriesdays.paises_info[c].confirmed.length - countriesdays.paises_info[c].recovered.length;
              countriesdays.paises_info[c] = Array(fill_count)
                .fill(0)
                .concat(countriesdays.paises_info[c].recovered);
            }
            //   if (all_recovers.paises_info[c].length >= cuba.length) {
            var c_temp = [c];
            var d_temp = ["Días"];
            for (var i = 1; i < countriesdays.paises_info[c].recovered.length; i++) {
              c_temp.push(countriesdays.paises_info[c].recovered[i]);
              d_temp.push("Día " + i);
            }
          ...
          var generate_recover_comparison = function(country) {
            return c3.generate({
              ...
              data: {
                x: dias[0],
                columns: [dias, recovers_cuba, recover_curve[recover_countryselected]["data"].slice(0, cuba.length)],
                type: "line",
                colors: {
                  Cuba: "#520924"
                }
          ...
          var generate_recover_graph = function() {
            return c3.generate({
              ...
              data: {
                x: "Días",
                columns: [recover_curve[recover_countryselected]["dias"], recovers_cuba, recover_curve[recover_countryselected]["data"]],
                type: "line",
                colors: {
                  Cuba: "#520924"
                }
          ...
          c3.generate({
            ...
            data: {
              x: dates[0],
              columns: [dates, dailySingle, dailyActive, dailySum],
              type: "line",
              colors: {
          ...
          comparison = c3.generate({
            ...
            data: {
              x: dias[0],
              columns: [dias, cuba, curves[countryselected]["data"].slice(0, cuba.length)],
              type: "line",
              colors: {
                Cuba: "#520924"
              }
          ...
          curve = c3.generate({
            ...
            data: {
              x: "Días",
              columns: [curves[countryselected]["dias"], cuba, curves[countryselected]["data"]],
              type: "line",
              colors: {
                Cuba: "#520924"
              }
          ...
          return {
            ...
            sujetos_riesgo: sujetos_riesgo,
            graves_numero: graves_numero,
            muertes_hoy: muertes_hoy,
      ...
      for (var i = 0; i < countrysorted2.length; i++) {
        var row = ("<tr><td>{ranking}</td>" + "<td>{country}</td>" + "<td>{cases}</td></tr>")
          .replace("{ranking}", i + 1)
