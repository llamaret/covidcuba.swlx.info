...
$.walker = {
    ...
    view: {
        update: function () {
            ...
            $('[data-content=ingresados]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=dia_pandem]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=graves]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=fallec_hoy]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=recupe_hoy]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=mortalidad]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=recuperacion]').html('<i class="fa fa-spinner fa-spin"></i>');
            $('[data-content=diagno_hoy]').html('<i class="fa fa-spinner fa-spin"></i>');

            const general_view = $locator.val() === 'cuba';
            let $generals = $('#recdist, #deadist, #tesmade-pcr, #tesacum, #topprov, #compari, #paises, #noshow, #topn-n-countries, #evomade, #proscurves, #testspor');
            if (general_view) {
...
function run_calculations() {
    ...
    $.walker.load("assets/data/paises-info-dias.json", function (countriesdays) {
        $.walker.load("assets/data/covid19-cuba.json", function (data) {
            $.walker.load("assets/data/provincias.geojson", function (provincias) {
                ...
                $.walker.load("assets/data/municipios.geojson", function (municipios) {
                    ...
                    function getAllCasesAndSimpleGraphics() {
                        ...
                        for (var day in data.casos.dias) {
                            cantidad_dias++;
                            if (data.casos.dias[day]["muertes_numero"]) {
                            muertes_hoy = data.casos.dias[day]["muertes_numero"];
                            } else {
                            muertes_hoy = 0;
                            }
                            if (data.casos.dias[day]["graves_numero"]) {
                            graves_numero = data.casos.dias[day]["graves_numero"];
                            } else {
                            graves_numero = 0;
                            }
                            if (data.casos.dias[day]["recuperados_numero"]) {
                            recuperados_hoy = data.casos.dias[day]["recuperados_numero"];
                            } else {
                            recuperados_hoy = 0;
                            }
                            if ("diagnosticados" in data.casos.dias[day]) {
                            casos_hoy = data.casos.dias[day]["diagnosticados"].length;
                            } else {
                            casos_hoy = 0;
                            }
                            if (data.casos.dias[day]["sujetos_riesgo"]) {
                            sujetos_riesgo = data.casos.dias[day]["sujetos_riesgo"];
                            } else {
                            sujetos_riesgo = 0;
                            }
                            if ('diagnosticados' in data.casos.dias[day]) {
                     ...
                     //       }
                        ...
                        return {"cases": cases, "deaths": deaths, "gone": gone, "recov": recov, "female": sex_female, "male": sex_male, "unknownsex": sex_unknown, "casos_hoy": casos_hoy, "sujetos_riesgo": sujetos_riesgo, "recuperados_hoy": recuperados_hoy, "graves_numero": graves_numero, "muertes_hoy": muertes_hoy, "cantidad_dias": cantidad_dias};
                    }
